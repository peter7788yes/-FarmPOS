...
{
	public class frmBatchPrintCommodity : Form
	{
		...
		private void displayTempMember()
		{
			...
			tableLayoutPanel1.RowCount = Program.commodityTemp.Count;
			string text = "1=1 and GDSNO in (";
			for (int i = 0; i < Program.commodityTemp.Count; i++)
			{
				text = text + "{" + i + "},";
			}
			...
			DataTable dataTable = (DataTable)DataBaseUtilities.DBOperation(Program.ConnectionString, TableOperation.Select, "*", "hypos_GOODSLST", text, "", null, Program.commodityTemp.ToArray(), CommandOperationType.ExecuteReaderReturnDataTable);
			if (dataTable.Rows.Count > 0)
			{
				for (int j = 0; j < dataTable.Rows.Count; j++)
				{
					...
					checkBox.Name = dataTable.Rows[j]["GDSNO"].ToString();
					checkBox.Text = dataTable.Rows[j]["GDSNO"].ToString();
					Label label = new Label();
					label.Text = dataTable.Rows[j]["GDNAME"].ToString();
					label.BackColor = Color.Transparent;
					label.Anchor = AnchorStyles.None;
					Label label2 = new Label();
					label2.Text = dataTable.Rows[j]["spec"].ToString();
					label2.BackColor = Color.Transparent;
					label2.Anchor = AnchorStyles.None;
					Label label3 = new Label();
					label3.Text = dataTable.Rows[j]["capacity"].ToString();
				...
				if (Program.commodityTemp.Count > 5)
				{
		...
		private void btn_removeSelected_Click(object sender, EventArgs e)
		{
			for (int i = 0; i < tableLayoutPanel1.RowCount; i++)
			{
				...
				if (checkBox.Checked)
				{
					Program.commodityTemp.Remove(checkBox.Text);
				}
			}
			displayTempMember();
		}
		...
		private void btn_printSelected_Click(object sender, EventArgs e)
		{
			if (Program.commodityTemp.Count > 0)
			{
				...
				if (list.Count > 0)
				{
					new Commodity_barcode(list, pageRecordCount).ShowDialog();
				}
				else
				{
					AutoClosingMessageBox.Show("無勾選列印商品");
				}
			}
			else
			{
				AutoClosingMessageBox.Show("無列印商品");
			}
		}
		...
		private void btn_printAll_Click(object sender, EventArgs e)
		{
			if (Program.commodityTemp.Count > 0)
			{
				...
				new Commodity_barcode(Program.commodityTemp, pageRecordCount).ShowDialog();
			}
			else
			{
				AutoClosingMessageBox.Show("無列印商品");
		...
		private void InitializeComponent()
		{
			...
			base.Name = "frmBatchPrintCommodity";
