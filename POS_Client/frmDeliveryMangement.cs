...
{
	public class frmDeliveryMangement : MasterThinForm
	{
		...
		private string selectStr = "a.editDate,a.status,a.DeliveryNo,a.CurSum,a.DeliveryDate,a.DeliveryCustomNo,a.sumDiscount,b.SupplierName";

		private string orderByStr = "a.editDate DESC";
		...
		private string getWhereStr()
		{
			...
			{
				return string.Format("a.vendorNo = b.SupplierNo and a.status in ({0}) and (b.SupplierName like {1} or b.SupplierIdNo like {1})", status, "'%" + text + "%'");
			}
			if (!string.IsNullOrEmpty(status))
			{
				return string.Format("a.vendorNo = b.SupplierNo and a.status in ({0})", status);
			}
			if (!"請輸入名稱或統一編號".Equals(text))
			{
				return string.Format("a.vendorNo = b.SupplierNo and (b.SupplierName like {0} or b.SupplierIdNo like {0})", "'%" + text + "%'");
			}
			return "a.vendorNo = b.SupplierNo";
		}
		...
		public void changePage(int page)
		{
			...
			for (int i = (pageNow - 1) * pageSize; i < pageNow * pageSize; i++)
			{
				if (i < dt.Rows.Count)
				{
					...
					dataGridView1.Rows.Insert(num, dt.Rows[i]["DeliveryDate"].ToString(), dt.Rows[i]["DeliveryNo"].ToString(), dt.Rows[i]["DeliveryCustomNo"].ToString(), dt.Rows[i]["SupplierName"].ToString(), (int.Parse(dt.Rows[i]["CurSum"].ToString()) - int.Parse(dt.Rows[i]["sumDiscount"].ToString())).ToString(), text, dt.Rows[i]["status"].ToString());
					num++;
				}
			}
		...
		{
			switchForm(new frmNewDeliveryOrder());
		}
		...
		private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			{
				string deliveryNo = dataGridView1[e.ColumnIndex, e.RowIndex].Value.ToString();
				dataGridView1["hidden_Status", e.RowIndex].Value.ToString();
		...
		private void btn_SearchPurchase_Click(object sender, EventArgs e)
		{
			...
			btn_statusCancel.ForeColor = Color.FromArgb(247, 106, 45);
			btn_statusCancel.BackColor = Color.White;
			frmSearchDelivery frmSearchDelivery = new frmSearchDelivery(this);
			frmSearchDelivery.Location = new Point(base.Location.X, base.Location.Y);
			frmSearchDelivery.Show();
			Hide();
		}

		public void UpdateDeliveryMangemnet(string sql)
		{
