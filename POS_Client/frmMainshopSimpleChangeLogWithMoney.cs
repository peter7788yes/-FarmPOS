...
{
	public class frmMainshopSimpleChangeLogWithMoney : Form
	{
		...
		public frmMainshopSimpleChangeLogWithMoney(string sellNo)
		{
			this.sellNo = sellNo;
			InitializeComponent();
		}
		...
		private void displayChangeLog()
		{
			...
			if (dataTable.Rows.Count > 0)
			{
				...
				{
					string text = dataTable.Rows[i]["sum"].ToString();
					...
					else if (dataTable.Rows[i]["ischange"].ToString() == "1" && dataTable.Rows[i]["isprint"].ToString() == "1")
					{
						...
						label3.Text = "商品退貨/補印收據(v" + (count + 1) + ")(變更後消費總額 " + text + ")";
						string[] strWhereParameterArray3 = new string[1]
						{
							dataTable.Rows[i]["sellLogId"].ToString()
						};
						DataTable dataTable2 = (DataTable)DataBaseUtilities.DBOperation(Program.ConnectionString, TableOperation.Select, "hdl.barcode as code,hdl.num,hdl.diffSellingPrice,hdl.diffNum,gd.*", "hypos_detailsell_log as hdl,hypos_GOODSLST as gd", "hdl.sellLogId = {0} and hdl.barcode = gd.GDSNO ", "", null, strWhereParameterArray3, CommandOperationType.ExecuteReaderReturnDataTable);
						if (dataTable2.Rows.Count > 0)
						{
							label3.Text += "\r\n";
							foreach (DataRow row in dataTable2.Rows)
							{
								label3.Text = label3.Text + row["code"].ToString() + row["GDName"].ToString() + row["spec"].ToString() + row["capacity"].ToString() + " (-" + row["diffNum"].ToString() + "/ -" + row["diffSellingPrice"].ToString() + ")\r\n";
							}
						}
					}
					else if (dataTable.Rows[i]["isprint"].ToString() == "1" && !string.IsNullOrEmpty(dataTable.Rows[i]["sum"].ToString()))
					{
						...
						DataTable dataTable3 = (DataTable)DataBaseUtilities.DBOperation(Program.ConnectionString, TableOperation.Select, "cash,Credit", "hypos_main_sell", "sellNo={0} ", "", null, strWhereParameterArray4, CommandOperationType.ExecuteReaderReturnDataTable);
						label3.Text = "新建銷售單(v1)\r\n銷售單總額 " + dataTable.Rows[i]["sum"].ToString() + "(現金 " + dataTable3.Rows[0]["cash"].ToString() + " / 賒帳 " + dataTable3.Rows[0]["Credit"].ToString() + ")";
					}
					else if (dataTable.Rows[i]["isprint"].ToString() == "1")
					{
						...
						int count2 = ((DataTable)DataBaseUtilities.DBOperation(Program.ConnectionString, TableOperation.Select, "*", "hypos_mainsell_log", "isprint = 1 and sellLogId < {0} and sellNo={1} ", "", null, strWhereParameterArray5, CommandOperationType.ExecuteReaderReturnDataTable)).Rows.Count;
		...
		private void InitializeComponent()
		{
			...
			base.Name = "frmMainshopSimpleChangeLogWithMoney";
